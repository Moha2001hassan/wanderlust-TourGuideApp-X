...
//        }
        ...
        String[] rating = resources.getStringArray(R.array.restoRating);
       ...
       // String[] url = resources.getStringArray(R.array.restoUrl);
        ...
        for (int i = 0; i < restoImgId.length; i++) {
            Restaurant restaurant = new Restaurant(restoImgId[i], name[i],
                    rating[i], type[i], phone[i], time[i], address[i]);
