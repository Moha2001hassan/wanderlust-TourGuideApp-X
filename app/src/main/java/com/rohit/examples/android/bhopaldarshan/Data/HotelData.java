...
public class HotelData {
    ...
    public static ArrayList<Hotel> fetchHotelData(Context context) {
        ...
        String[] rating = resources.getStringArray(R.array.hotelRating);
        ...
        String[] hotelPrice = resources.getStringArray(R.array.hotelPrice);

        //Mapping data set with recycler view items accordingly
        for (int i = 0; i < hotelImgId.length; i++) {
            Hotel hotel = new Hotel(hotelImgId[i], name[i], rating[i], phone[i], type[i], address[i], hotelPrice[i]);
            hotels.add(hotel);
        }
        return hotels;
    }
